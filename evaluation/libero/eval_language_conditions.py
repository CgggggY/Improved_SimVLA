...
def get_selected_task_ids(task_suite_name, task_ids_map):
    if not task_ids_map:
        return None
    return task_ids_map.get(task_suite_name, None)


def choose_eval_prompt(task_description, mode, para_map, wrong_map, generic_map):
    if mode == "clean":
        return task_description
    if mode == "paraphrase":
        variants = para_map.get(task_description, [])
        return random.choice(variants) if variants else task_description
    if mode == "wrong_target":
        variants = wrong_map.get(task_description, [])
        return random.choice(variants) if variants else task_description
    if mode == "generic":
        variants = generic_map.get(task_description, [])
        return random.choice(variants) if variants else task_description
...
def eval_libero(
    ...
    mode: str = "clean",
    para_map: Optional[Dict[str, List[str]]] = None,
    wrong_map: Optional[Dict[str, List[str]]] = None,
    generic_map: Optional[Dict[str, List[str]]] = None,
    ...
    if selected_task_ids is None:
        ...
        task_ids = [task_id for task_id in selected_task_ids if 0 <= task_id < num_tasks]
        if len(task_ids) < len(selected_task_ids):
            print("Warning: some selected task ids are out of range and will be skipped.")
    ...
    for task_id in tqdm(task_ids, desc="Tasks"):
        ...
        eval_prompt = choose_eval_prompt(
            task_description,
            mode=mode,
            para_map=para_map,
            wrong_map=wrong_map,
        ...
        print(f"[Task {task_id}] mode={mode} | orig='{task_description}' | eval='{eval_prompt}'")
        ...
        for ep in tqdm(range(num_trials), desc=f"{task_description[:30]}...", leave=False):
            ...
            while t < max_steps + NUM_STEPS_WAIT:
                try:
                    ...
                    action = client.step(obs_dict, eval_prompt)
        ...
        per_task_results.append(
            {
                ...
                "success_rate": task_successes / max(num_trials, 1),
            }
        )
...
def main():
    ...
    parser.add_argument("--mode", type=str, default="clean",
                        choices=["clean", "paraphrase", "generic", "wrong_target"])
    parser.add_argument("--para_json", type=str, default=None,
                        help="Path to task_to_paraphrases json")
    parser.add_argument("--wrong_json", type=str, default=None,
                        help="Path to task_to_wrong_targets json")
    parser.add_argument("--generic_json", type=str, default=None,
                        help="Path to generic_instructions json")
    parser.add_argument("--task_ids_json", type=str, default=None,
                        help="Path to selected_task_ids json")
    parser.add_argument("--save_result", type=str, default=None,
                        help="Path to save evaluation result json")

    args = parser.parse_args()
    random.seed(args.seed)
    para_map = load_json_or_default(args.para_json, {})
    wrong_map = load_json_or_default(args.wrong_json, {})
    generic_map = load_json_or_default(args.generic_json, {})
    task_ids_map = load_json_or_default(args.task_ids_json, {})
    selected_task_ids = get_selected_task_ids(args.task_suite, task_ids_map)
